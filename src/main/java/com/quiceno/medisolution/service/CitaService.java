...
/**
 * Lógica de negocio para agendamiento de Citas.
 * Realiza las verificaciones de agenda cruzada y disponibilidad.
 */
@Service
public class CitaService {
    ...
    /**
     * Obtiene una lista de citas filtrada dinámicamente por múltiples criterios y
     * rangos de fechas.
     */
    ...
    public CitaDTO guardar(CitaDTO dto) {
        ...
        if (medicoOcupado) {
            throw new IllegalArgumentException(
                    "Error: El médico para la fecha establecida estará ocupado, por favor seleccione otra fecha.");
        }

        // Validar si el paciente tiene otra cita programada al mismo tiempo que la que
        // se intenta ingresar
        boolean pacienteOcupado = citaRepository.existsByPacienteIdAndFecha(dto.getPacienteId(), dto.getFecha());
        if (pacienteOcupado) {
            throw new IllegalArgumentException(
                    "Error: El paciente ya tiene agendada una cita para la fecha y hora ingresada.");
        }
    ...
    public CitaDTO actualizar(CitaDTO dto) {
        ...
        if (dto.getPacienteId() != null && !dto.getPacienteId().equals(citaActualizar.getPaciente().getId())) {
            citaActualizar.setPaciente(pacienteRepository.findById(dto.getPacienteId()).orElseThrow(
                    () -> new ResourceNotFoundException(
                            "Error: El paciente que intenta reasignar a esta cita, no existe.")));
        }
        ...
        if (dto.getMedicoId() != null && !dto.getMedicoId().equals(citaActualizar.getMedico().getId())) {
            citaActualizar.setMedico(medicoRepository.findById(dto.getMedicoId()).orElseThrow(
                    () -> new ResourceNotFoundException(
                            "Error: El médico que intenta reasignar a esta cita, no existe.")));
        }

        // 4. Validamos disponibilidad SOLO si la fecha, el médico o el paciente
        // cambiaron.
        // Usamos el ID actual (dto.getId()) para ignorar la cita que estamos editando.
        boolean medicoOcupado = citaRepository.existsByMedicoIdAndFechaAndIdNot(dto.getMedicoId(), dto.getFecha(),
                dto.getId());
        boolean pacienteOcupado = citaRepository.existsByPacienteIdAndFechaAndIdNot(dto.getPacienteId(), dto.getFecha(),
                dto.getId());

        // Usamos OR (||) y Custom Exception
        if (medicoOcupado || pacienteOcupado) {
            // Sobre tu duda de la excepción: DuplicateResourceException es válida, o
            // IllegalArgumentException.
            throw new DuplicateResourceException(
                    "Error: El horario que designó para la cita ya está ocupado en la agenda del médico o el paciente.");
        }
        ...
        if (dto.getEspecialidadId() != null
                && !dto.getEspecialidadId().equals(citaActualizar.getEspecialidad().getId())) {
